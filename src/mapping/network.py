"""Network connectivity and object-to-node crosswalks."""
...
def largest_connected_network(network: NetworkData) -> NetworkData:
    """Return the largest branch-connected subgraph of the standard network."""

    node_uids = set(network.nodes["uid"].dropna())
    branches = network.branches.loc[
        network.branches["from_uid"].isin(node_uids)
        & network.branches["to_uid"].isin(node_uids)
        & network.branches["from_uid"].ne(network.branches["to_uid"])
    ].copy()
    graph = nx.Graph()
    graph.add_edges_from(branches[["from_uid", "to_uid"]].itertuples(
        index=False,
        name=None,
    ))
    if graph.number_of_nodes() == 0:
        raise ValueError("Network has no valid branch-connected nodes.")
    largest = max(nx.connected_components(graph), key=len)
    nodes = network.nodes.loc[network.nodes["uid"].isin(largest)].copy()
    branches = branches.loc[
        branches["from_uid"].isin(largest) & branches["to_uid"].isin(largest)
    ].copy()
    nodes["in_largest_connected_graph"] = True
    branches["in_largest_connected_graph"] = True
    nodes.attrs = network.nodes.attrs.copy()
    branches.attrs = network.branches.attrs.copy()
    result = NetworkData(nodes.reset_index(drop=True), branches.reset_index(drop=True))
    validation = nx.Graph()
    validation.add_nodes_from(result.nodes["uid"])
    validation.add_edges_from(result.branches[["from_uid", "to_uid"]].itertuples(
        index=False,
        name=None,
    ))
...
def map_to_nodes(
    ...
    *,
    ...
    """Map objects to electrical nodes with deterministic random tie-breaking."""

    candidates = nodes.copy()
    if node_classes:
        candidates = candidates.loc[candidates["class"].isin(node_classes)].copy()
    if candidates.empty:
        raise ValueError("No eligible network nodes remain for mapping.")
    left = objects.copy()
    ...
    if method == "cell":
        ...
        raise ValueError("Node mapping method must be 'geometry' or 'cell'.")
    ...
    joined = pd.concat(matches, ignore_index=True).sort_values(
        ["_row", "uid"],
        kind="stable",
    )
    ...
    selected = selected.rename(columns={"uid": "node_uid"})
    ...
    return selected[[
        ...
        "spatial_uid_right": "node_spatial_uid",
        "admin_uid_left": "source_admin_uid",
        "admin_uid_right": "node_admin_uid",
    })


def attach_node_mapping(
    objects: gpd.GeoDataFrame,
    mapping: pd.DataFrame,
    *,
    source_uid_column: str,
) -> gpd.GeoDataFrame:
    """Add a one-to-one electrical-node mapping to an object table."""

    if mapping[source_uid_column].duplicated().any():
        raise ValueError(f"Node mapping is not unique by {source_uid_column!r}.")
    columns = mapping[[
        source_uid_column,
        "node_uid",
        "mapping_method",
        ...
        "mapping_method": "node_mapping_method",
        "distance_km": "node_distance_km",
        "same_admin": "node_same_admin",
    })
    result = objects.merge(columns, on="uid", how="left", validate="one_to_one")
    if result["node_uid"].isna().any():
        raise ValueError("Some mapped objects do not have an electrical node.")
    result = gpd.GeoDataFrame(result, geometry="geometry", crs=objects.crs)
    result.attrs = objects.attrs.copy()
    return result


def attach_node_coordinates(
    data: xr.Dataset,
    mapping: pd.DataFrame,
    *,
    source_uid_column: str,
) -> xr.Dataset:
    """Add a one-to-one electrical-node mapping as coordinates along uid."""
    ...
    if indexed["node_uid"].isna().any():
        raise ValueError("Some mapped time-series locations have no electrical node.")
    return data.assign_coords({
        "node_uid": ("uid", indexed["node_uid"].astype(str).to_numpy()),
        "node_mapping_method": (
            "uid", indexed["mapping_method"].astype(str).to_numpy()
        ),
        "node_distance_km": ("uid", indexed["distance_km"].to_numpy()),
        "node_same_admin": ("uid", indexed["same_admin"].to_numpy()),
        "node_spatial_uid": (
            "uid", indexed["node_spatial_uid"].astype(str).to_numpy()
        ),
        "node_admin_uid": (
            "uid", indexed["node_admin_uid"].astype(str).to_numpy()
        ),
    })
...
def _nearest(left: gpd.GeoDataFrame, right: gpd.GeoDataFrame) -> pd.DataFrame:
    return pd.DataFrame(gpd.sjoin_nearest(
        ...
        right[["uid", "spatial_uid", "admin_uid", "geometry"]],
        how="left",
